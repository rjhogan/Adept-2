...
you did not link to an external \Adept\ library. Since \Adept\ depends
on BLAS for matrix multiplication, you will still need to link to a
functioning BLAS library (such as OpenBLAS). By default,
\code{adept\_arrays.h} does not enable LAPACK (needed for matrix
inversion and solving linear systems of equations); to enable it, edit
the line near the top of \code{adept\_source.h} containing
\code{HAVE\_LAPACK}, and link against a functioning LAPACK library.  A
demonstration of the use of \code{adept\_source.h} is in the
...
\item On Intel architectures, \Adept\ will vectorize expressions if
  ...
  elementary mathematical operators, (2) the arrays in the expression
  are either all of type \code{float} or all of type \code{double},
  (3) all the arrays in the expression must have their final dimension
  increasing in memory with no stride, and (4) none of the arrays are
  active. If AVX is enabled (\code{-mavx}) then four \code{double}s or
  eight \code{float}s will be operated on at once, otherwise if SSE2
  is enabled (\code{-msse2}) then two \code{double}s or four
  \code{float}s will be operated on at once.
...
otherwise (appropriate for at least the GNU, Intel, Sun and IBM
