...
\lstset{language=C++,
  ...
  emph={adouble,xdouble,Stack,adept,Array,FixedArray,Vector,aVector,Matrix,aMatrix,Array3,aArray3,intVector,boolVector,floatVector,adept,SpecialMatrix,SquareMatrix,aSquareMatrix,SymmMatrix,aSymmMatrix,UpperMatrix,LowerMatrix,IntVector},
...
% Title material
\title{\Adept\ C++ Software Library: User
  Guide} \author{Robin J. Hogan\\ \emph{European Centre for Medium
    Range Weather Forecasts, Reading, UK}\\ \emph{and School of
    Mathematical and Physical Sciences, University of Reading, UK,}}

\date{Document version 1.9.8 (April 2016) applicable to \Adept\ version
  1.9.8 \thanks{This document is copyright \copyright\ Robin J. Hogan
    2013--2016.  Permission is granted to copy, distribute and/or
...
functions if available\footnote{This decision may puzzle some readers,
  ...
  is rather well designed. All references to Fortran in this document
  imply the 1990 (or later) standard.}. This design makes \Adept\ a
good choice if you have Fortran code that you wish to convert to C++.
...
\item[Array reduction operations.] The functions \code{sum},
  ...
  expressions. \code{count}\footnote{Not yet implemented.} returns the
  number of \code{true} elements in a boolean expression.
% The function
...
\code{Vector}, \code{Matrix} and \code{Array3} provide inactive arrays
of type \code{Real} and rank 1--3. The corresponding active types are
\code{aVector}, \code{aMatrix} and \code{aArray3}. Vectors of other
...
\end{lstlisting}
%POINT TO EXISTING DATA; ExpressionSize
...
rank other than 2.
%
\citem{diag\_vector(i)} When applied to a square rank-2 $n$-by-$n$
...
\label{sec:passing}
How can large arrays be returned efficiently from a function?  If
array \code{A} is created in a function and then returned with
\code{return A}, the receiving function invokes a copy constructor to
copy \code{A} into an array in that function, followed by calling the
destructor of \code{A}. It is unnecessary to perform a deep copy when
\code{A} is about to be destructed; it is enough for the receiving
array to simply copy the data members of \code{A}, i.e.\ little more
than the dimensions of the array and pointers to the data and the
\code{Storage} object. This is how copy constructors are implemented
in \Adept. Consider the following function to square the elements of a
matrix:
\begin{lstlisting}
...
\end{lstlisting}
When an empty matrix is constructed using ``\code{Matrix out}'', it is
placed on the stack but no memory is allocated on the heap for the
actual contents of the matrix via a \code{Storage} object. This is
done on the next line when it is assigned to \code{in*in}.  At the
\code{return} statement, matrix \code{out} is received by the copy
constructor of matrix \code{B}, so a shallow copy is performed. This
means that the description of matrix \code{out} is copied to \code{B},
including a pointer to the \code{Storage} object. Matrix \code{out} is
then destructed, with the net result being that \code{B} has
``stolen'' the data in the matrix from \code{out} without it having
been copied, thus avoiding unnecessary allocation of memory on the
heap, followed by copying and deallocation.

The fact that \Adept\ copy constructors perform shallow copies leads
to two sorts of behaviour that users may not be expecting. Firstly, if
an array is initialized from another array in either of the following
two ways:
\begin{lstlisting}
 Matrix M(3,4);
...
\end{lstlisting}
Secondly, arrays passed to functions ``by value'' behave as if they
...
\begin{lstlisting}
 ...
 sqr_in_place2(B); 
\end{lstlisting}
These two functions behave the same to the user: the second function
creates a shallow copy of \code{B} in \code{A}, and thus any
modifications to \code{A} are also made to \code{B}.

The question of how to implement copy constructors is a classical C++
problem, solved in C++11 with ``move semantics'': the compiler can
tell when an object is about to be destructed, in which case it
performs a shallow ``move'' rather than a deep ``copy''.  The
\Adept\ approach is compatible with older C++ compilers that do not
...
to their argument, and operate in the same way as Fortran functions
of the same name.  Consider the \code{sum} function, which can be used
...
argument and return an array whose rank is one less than the first argument:
\begin{lstlisting}
 Array A(3,4);
...
otherwise), while \code{count}\footnote{Not yet implemented.} returns
the number of \code{true} elements.  Each of these can work on an
individual dimension as with \code{sum} and friends.
...
function and returns an \code{IntVector} containing indices to the
...
\begin{lstlisting}
 ...
 v(index) = 2.0 * u(index);
\end{lstlisting}
This will work if none if no \code{true} elements are found:
\code{find} will return an empty array, and when \code{v} is indexed
by an empty vector, no action will be taken.  In general, \code{find}
is less efficient than \code{where}.
...
illustrated here:\footnote{A drawback of the \code{**} interface with
  ...
  expression will not give the same answer as \code{A**(v**B)}. I may
